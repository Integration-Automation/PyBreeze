...
從瀏覽器開發者工具貼上一段 `curl` 指令，再選擇輸出目標。解析器能處理方法、URL、標頭、本文、Basic 驗證、`-G` 與 `--url-query` 查詢參數、`-F` multipart 欄位（上傳檔案會變成 `files=open(...)`）、`--json` 簡寫、`-d @file` 本文，以及多行接續。其他需要參數的 curl 選項也會把參數吃掉，不會被誤當成 URL。重複的 `-H` 值會照 HTTP 的方式合併（cookie 用 `; `，其他用 `, `），而不是默默只留下最後一個。方法和 curl 實際送出的一樣：有本文就是 POST、`-I` 是 HEAD，除非 `-X` 指定了方法（`-X GET -d ...` 仍是帶本文的 GET）。過程中不會執行任何東西——純粹是解析。
