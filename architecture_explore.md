...
| `header_tools/` | `header_analyzer.py`(369) 安全稽核（照 OWASP HTTP Headers Cheat Sheet：CSP 的 `frame-ancestors` 視同 `X-Frame-Options`，`X-XSS-Protection: 0` 不報，`_DEPRECATED_HEADERS` 是現行瀏覽器已忽略的標頭）；`header_merge.py` 依 HTTP 規則合併重複 header（Cookie 用 `; ` 其餘用 `, `） |
