...
| `ssh_key_loader.py` | 依序嘗試各種私鑰型別，回傳第一個能解析的；paramiko 不讀的 PKCS#8（`BEGIN PRIVATE KEY`／`BEGIN ENCRYPTED PRIVATE KEY`）由 `cryptography` 讀進來、在記憶體裡轉成 OpenSSH 格式再交給 paramiko；都不行時 `unloadable_key_reason()` 分辨是密語沒給／給錯（檔案有加密，而且給的密語解不開它；用 `cryptography` 試解）還是不支援的私鑰（例如加密的 DSA） |
