...
| `ssh_file_viewer_widget.py` (711) | `SFTPClientWrapper` + `SSHFileTreeManager`：延遲載入的遠端檔案樹、右鍵選單（重新整理/建資料夾/改名/刪除/下載/上傳）、目錄優先 + 自然排序。每個 SFTP 操作都先 `_require_connection()`；連線交給 `SshConnectThread`，成功後才列出根目錄；下載／上傳交給 `SftpTransferThread(QThread)`（傳輸沒有自己的逾時，跑在 UI 執行緒會把整個 IDE 凍到傳完），一次只允許一個，`closeEvent` 等它結束再關掉 SFTP 連線 |
