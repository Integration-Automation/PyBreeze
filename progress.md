...
- **#32** Bash `$'...'` quoting, which browsers' Copy as cURL (bash) uses for bodies holding newlines or quotes, is not understood (`curl_parser._tokenize`): `--data-raw $'{"m":"a\nb"}'` keeps a leading `$` so the body is never recognised as JSON, and `$'it\'s'` is refused as badly quoted.
